...
public class NhaCCService {
    ...
    public void delete(String maNCC) {
        try {
            sql = "DELETE FROM CTPHIEUNHAP WHERE MAPN IN (SELECT MAPN FROM PHIEUNHAP WHERE MANCC = ?)";
            ps = con.prepareStatement(sql);
            ps.setString(1, maNCC);
            ps.executeUpdate();
            sql = "DELETE FROM Phieunhap WHERE MANCC = ?";
            ps = con.prepareStatement(sql);
            ps.setString(1, maNCC);
            ps.executeUpdate();
            sql = "DELETE FROM Sach_NhaCungCap WHERE MANCC = ?";
            ps = con.prepareStatement(sql);
            ps.setString(1, maNCC);
            ps.executeUpdate();
