...
public class JDSP extends javax.swing.JDialog {
    ...
    private TacGiaDAO tgBUS = new TacGiaDAO();
    ...
    public String tenSach() {
        return txTenSach.getText() + "%" + txMaSach.getText();
    }
 ...
 /* Create and display the dialog */
     ...
     * DSSP***********************************
     */
    public void outModel(DefaultTableModel model, List<SachDTO> s) {
        Vector row;
        model.setRowCount(0);
        for (SachDTO sa : s) {
            row = new Vector();
    ...
    public void LoadDSSP() {
        ...
        try {
            List<SachDTO> dss = new ArrayList<>();
            dss = sachBUS.selectAll();
            outModel(model, dss);
        } catch (Exception ex) {
...
// tìm*****************************************
    ...
    public void listTG(JComboBox cmb) throws Exception {
        List<TacGiaDTO> tg = tgBUS.selectAll();
        for (TacGiaDTO a : tg) {
...
//            search=bus.selectById(txTMaSach.getText(),
//                    "Không",
