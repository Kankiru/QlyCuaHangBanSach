...
public class BanHangGUI extends javax.swing.JFrame {
 private int DEFALUT_WIDTH;
    ...
    private KhachHangDAO khBUS = new KhachHangDAO();
    private NhanVienDAO nvBUS = new NhanVienDAO();
    private KhuyenMaiDAO voucherDAO = new KhuyenMaiDAO();
    private ArrayList<ChiTietHoaDonDTO> DSCT = new ArrayList<>();
    private ArrayList<KhuyenMaiDTO> vch = new ArrayList<>();
    /**
...
public BanHangGUI(int width, String userID) {
    ...
    private void initComponents() {
        ...
        jPanel1.setBorder(javax.swing.BorderFactory.createBevelBorder(javax.swing.border.BevelBorder.RAISED, new java.awt.Color(248, 248, 248), new java.awt.Color(204, 204, 204), new java.awt.Color(204, 204, 204), new java.awt.Color(204, 204, 204)));
        ...
        btTaoHD.setBackground(new java.awt.Color(51, 255, 51));
        ...
        btXacNhan.setBackground(new java.awt.Color(255, 0, 51));
        ...
        txThanhTien.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseEntered(java.awt.event.MouseEvent evt) {
                txThanhTienMouseEntered(evt);
            }
        });
            ...
            .addGroup(panelCTHDLayout.createSequentialGroup()
                .addGroup(panelCTHDLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                    .addGroup(panelCTHDLayout.createSequentialGroup()
                        ...
                        .addGap(18, 18, 18)
                        .addGroup(panelCTHDLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addComponent(btMaSach, javax.swing.GroupLayout.Alignment.TRAILING, javax.swing.GroupLayout.DEFAULT_SIZE, 40, Short.MAX_VALUE)
        ...
        btMaGG.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btMaGGActionPerformed(evt);
            }
        });
                ...
                try {
                    ...
                    for (SachDTO s : sa) {
                        if (txMaSach.getText().toUpperCase().equals(s.getMaSach().toUpperCase())) {
                            ...
                            txDonGia.setText(String.valueOf(s.getGiaMua()));
                            Image newImage;
                            //            try{
    ...
    private void txThanhTienMouseEntered(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_txThanhTienMouseEntered
        // TODO add your handling code here:
    }//GEN-LAST:event_txThanhTienMouseEntered

    private void btMaGGMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btMaGGMouseClicked
        JDGG gg = new JDGG();
        String s = gg.getMaVoucher();
        txMaGG.setText(s);
    }//GEN-LAST:event_btMaGGMouseClicked

    private void btMaGGActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btMaGGActionPerformed
        // TODO add your handling code here:
    }//GEN-LAST:event_btMaGGActionPerformed
...
//bt Tạo hóa đơn*******************************************************************
    ...
    public void TaoHD() {
        ...
        for (NhanVienDTO nv : nvDTO) {
            if (String.valueOf(nv.getIdNV()).equals(txMaNV.getText())) {
                dem++;
            }
        }
        if (dem == 0) {
        ...
        KhachHangDAO khBUS = new KhachHangDAO();
        List<KhachHangDTO> khDTO = new ArrayList<>();
        khDTO = khBUS.selectAll();
        ...
        for (KhachHangDTO kh : khDTO) {
            if (kh.getMaKH().equals(txMaKH.getText())) {
                dem1++;
            }
        }
        if (dem1 == 0) {
...
//bt Thêm ***************************************************************************************
    ...
    public void Them() {
        ...
        // Giá
        int gia = Integer.parseInt(txDonGia.getText());
        // Giảm giá
        int giamgia = 0;
        int giam = 0;
        if (!txMaGG.getText().equals("")) {
            // Lấy phần trăm giảm giá từ voucher
            KhuyenMaiDTO voucher = voucherDAO.getVoucherByMaVoucher(txMaGG.getText());
            if (voucher != null) {
                giam = (int) voucher.getGiamGia();
                giamgia = s * gia * giam / 100;
            }
        }
        // Thành tiền
        int thanhtien = s * gia - giamgia;
        // Kiểm tra tồn tại
        boolean flag = true;
        for (ChiTietHoaDonDTO ct : DSCT) {
        ...
        if (flag) {
            ...
            DSCT.add(new ChiTietHoaDonDTO("LSG1", txMaHD.getText(), txMaSach.getText(), txTenSach.getText(), s, gia, giamgia, thanhtien));
...
// tổng tiền
    ...
    public void GiamGiaThanhTien() {
        double phantramgiamgia = 0;

        for (KhuyenMaiDTO voucher : vch) {
            phantramgiamgia = phantramgiamgia + (double) voucher.getGiamGia();
        }
        txGiamGia.setText(String.valueOf(phantramgiamgia));
    }
// bt xóa****************************************************************************
...
//                          }
                    ...
                    ct.setThanhTien((int) thanhtien);
                    ct.setGiamGia((int) giamgia);
                    ct.setSoLuong(sl);
                }
            }
            outModel(model, DSCT);
...
//bt xác nhận hóa đơn *****************************************************************
    ...
    public void XacNhan() {
        ...
        float TongTien = Float.parseFloat(txThanhTien.getText());
        float GiamGia = Float.parseFloat(txGiamGia.getText());
        float ThanhTien = Float.parseFloat(txTongTien.getText());

        // Kiểm tra xem người dùng đã nhập mã voucher hay chưa
        if (!MaGG.isEmpty()) {
            // Nếu đã nhập mã voucher, thực hiện tính toán tiền giảm giá dựa trên phần trăm giảm giá của voucher
            KhuyenMaiDAO voucherDAO = new KhuyenMaiDAO();
            KhuyenMaiDTO voucher = voucherDAO.selectById(MaGG);
            if (voucher != null) {
                // Lấy phần trăm giảm giá từ voucher
                double phanTramGiamGia = voucher.getGiamGia();
                // Tính tiền giảm giá dựa trên tổng tiền của hóa đơn và phần trăm giảm giá
                double tienGiamGia = (TongTien * phanTramGiamGia) / 100;
                // Cập nhật lại tổng tiền và thành tiền sau khi áp dụng giảm giá
                ThanhTien -= tienGiamGia;
                GiamGia += tienGiamGia;
                // Hiển thị lại tổng tiền và thành tiền trên giao diện BanHangGui
                txGiamGia.setText(String.valueOf(GiamGia));
                txThanhTien.setText(String.valueOf(ThanhTien));
            }
        }
        ...
        // Cập nhật số lượng sách trong kho và thêm chi tiết hóa đơn vào cơ sở dữ liệu
        for (ChiTietHoaDonDTO ct : DSCT) {
            sachBUS.updateSL(ct.getMaSach(), ct.getSoLuong());
            ctBUS.Them(ct);
        }

        // Làm sạch các trường nhập liệu và hiển thị lại giao diện
        txMaHD.setText(hdBUS.AutoMaHD());
        HuyHD(true);
    }

    public javax.swing.JTextField getTxTongTien() {
        return txTongTien;
    }

    public void updateTotalBill(double tienGiamGia) {
        // Lấy tổng tiền hiện tại từ field hoặc biến tương ứng trên giao diện
        double tongTienHienTai = Double.parseDouble(txTongTien.getText());
        // Trừ đi số tiền giảm giá từ voucher
        double tongTienSauGiamGia = tongTienHienTai - tienGiamGia;
        // Cập nhật lại tổng tiền trên giao diện
        txTongTien.setText(String.valueOf(tongTienSauGiamGia));
        // Gọi phương thức cập nhật tổng tiền và các thông tin liên quan khác nếu cần
        // Có thể thêm các logic khác ở đây tùy theo yêu cầu của ứng dụng
    }
