...
public class NhapSach_Service {
    ...
    public List<NhapSach> getAll(){
        List<NhapSach> listNs = new ArrayList<>();
        ...
        try {
            ...
            rs = ps.executeQuery();
            while(rs.next()){
               NhapSach ns = new NhapSach(rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getInt(5));
                listNs.add(ns);
            }
            return listNs;
        } catch (Exception e) {
...
//        return count;
//    }
    public int upDate(String ma,NhapSach ns){
    ...
    public List<NhapSach> timkiemMPN(String maPN){
        try {
            List<NhapSach> ListN = new ArrayList<>();
    ...
    public List<NhapSach> timkiemMNCC(String maNCC){
        try {
            List<NhapSach> ListN = new ArrayList<>();
            sql = "select * from PHIEUNHAP where MANCC like ?";
            con = DBConnect.getConnection();
            ps = con.prepareStatement(sql);
    ...
    public List<NhapSach> timkiemMNV(String maNV){
        try {
            List<NhapSach> ListN = new ArrayList<>();
