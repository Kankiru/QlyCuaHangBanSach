...
public class QlySach_Service {
    ...
    public List<QlySach> timkiem2(String maSach){
        try {
            
            List<QlySach> ListS = new ArrayList<>();
            sql = "SELECT MASACH,MANXB,THELOAI,MATG,TENSACH,NAMXUATBAN,SOLUONG,GIAMUA,ANH FROM SACH WHERE MASACH LIKE ?";
             con = DBConnect.getConnection();
            ps = con.prepareStatement(sql);
    ...
    public List<QlySach> timkiem3(String maNXB){
        try {
            
            List<QlySach> ListS = new ArrayList<>();
            sql = "SELECT MASACH,MANXB,THELOAI,MATG,TENSACH,NAMXUATBAN,SOLUONG,GIAMUA,ANH FROM SACH WHERE MANXB LIKE ?";
             con = DBConnect.getConnection();
            ps = con.prepareStatement(sql);
    ...
    public List<QlySach> timkiem4(String maTG){
        try {
            
            List<QlySach> ListS = new ArrayList<>();
            sql = "SELECT MASACH,MANXB,THELOAI,MATG,TENSACH,NAMXUATBAN,SOLUONG,GIAMUA,ANH FROM SACH WHERE MATG LIKE ?";
             con = DBConnect.getConnection();
            ps = con.prepareStatement(sql);
            ps.setObject(1, "%" + maTG + "%");
            rs = ps.executeQuery();
        ...
        public List<QlySach> timkiem5(String maTL){
        try {
            
            List<QlySach> ListS = new ArrayList<>();
            sql = "SELECT MASACH,MANXB,THELOAI,MATG,TENSACH,NAMXUATBAN,SOLUONG,GIAMUA,ANH FROM SACH WHERE THELOAI LIKE ?";
             con = DBConnect.getConnection();
            ps = con.prepareStatement(sql);
            ps.setObject(1, "%" + maTL + "%");
            rs = ps.executeQuery();
